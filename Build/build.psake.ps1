...
#
# The Install task simply copies the $OutDir to your profile's Modules folder.
#
...
Task Test -depends Build -requiredVariables TestRootDir, ModuleName, CodeCoverageEnabled, CodeCoverageOutputFormat, CodeCoverageOutputPath, CodeCoverageFiles, TestOutputEnabled, TestOutputFormat, TestOutputFile, SrcRootDir {
    ...
    try {
        ...
        if ($TestOutputEnabled) {
            ...
            $config.TestResult.OutputFormat = $TestOutputFormat
        ...
        if ($CodeCoverageEnabled) {
            ...
            $config.CodeCoverage.OutputPath = $CodeCoverageOutputPath
            $config.CodeCoverage.Path = $CodeCoverageFiles
        }
        ...
        if ($CodeCoverageEnabled) {
            if($testResult.CodeCoverage.NumberOfCommandsAnalyzed)
            {
            ...
            if(Test-Path $CodeCoverageOutputPath)
            {
                ...
                (Get-Content $CodeCoverageOutputPath) -replace 'sourcefile name="([^"]+)"', "sourcefile name=""$ResolvedPath/`$1""" | Set-Content $CodeCoverageOutputPath
            }
        }
    }
    finally {
