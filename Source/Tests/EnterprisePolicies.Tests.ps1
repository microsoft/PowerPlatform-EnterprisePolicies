BeforeDiscovery {
    ...
    $moduleFiles = Get-ChildItem -Path "$Global:ModuleManifestPath\Private\*.ps1"
    $scriptFiles = $Global:ModuleScriptsPaths | ForEach-Object { Get-ChildItem -Path "$_\*.ps1" -Recurse }
}
